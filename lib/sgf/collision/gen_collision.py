...
class Line(object):
	...
	def draw(self,Surface):
		pygame.draw.line(Surface,self.color,self.p1,self.p2,4)
	...
	def calculateYAxisIntercept(self,p,m):
		return p[1] - (m * p[0])
	...
	def getIntersectPoint(self,p1,p2,p3,p4):
		...
		if m1 != m2:
			...
			if m1 is not None and m2 is not None:
				# neither is vertical
				b1 = self.calculateYAxisIntercept(p1,m1)
				b2 = self.calculateYAxisIntercept(p3,m2)
	...
	def calculateIntersectPoint(self,p1,p2,p3,p4):
		"""Tests if two lines are colliding based on the points passed in
		Returns None if there is no intersection"""
	
		p = self.getIntersectPoint(p1,p2,p3,p4)
