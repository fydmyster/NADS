...
class Player(object):
	...
	def calculateFireAngleRevised(self):
		'''
		self.fire_angle = self.fire_angle % 360
		
		
		if self.left_is_down and self.up_is_down:
			
			xvec1 = math.cos(225 * (math.pi/180))
			yvec1 = math.sin(225 * (math.pi/180))
		
			xvec2 = math.cos(self.fire_angle * (math.pi/180))
			yvec2 = math.sin(self.fire_angle * (math.pi/180))
		...
		elif self.right_is_down and self.up_is_down:
			
			xvec1 = math.cos(315 * (math.pi/180))
			yvec1 = math.sin(315 * (math.pi/180))
		...
		if self.left_is_down:
			...
			xvec1 = math.cos(180 * (math.pi/180))
			yvec1 = math.sin(180 * (math.pi/180))
			...
			xvec1 = math.cos(360 * (math.pi/180))
			yvec1 = math.sin(360 * (math.pi/180))
		...
		elif self.up_is_down:
		 
			xvec1 = math.cos(270 * (math.pi/180))
			yvec1 = math.sin(270 * (math.pi/180))
	...
	def calculateFireAngle(self):
		...
		# angle between player rect center and shooter_rect center
		...
		angle = math.atan2(yvec,xvec) * (180/math.pi)
		
		return (angle,(xvec,yvec))
	...
	def slash(self):
		pass
		'''
		if not self.animator.anim_started:
			if self.last_dir == 0:
				self.animator.playRepeat(self.HIT_LEFT)
				
				# add effect
				new_effect = Effect(self.x - 8,self.y,"images\\slash_left.png")
				self.effects_list.append(new_effect)
				
			elif self.last_dir == 1:
				self.animator.playRepeat(self.HIT_RIGHT)
				
				# add effect
				new_effect = Effect(self.x + 8,self.y,"images\\slash_right.png")
				self.effects_list.append(new_effect)
		'''
	...
	def update(self,timepassed):
		...
		self.calculateFireAngleRevised()
		self.firing_angle = self.fire_angle
		
		# calculate pointer image position
		p_xpos = self.magic_orb.rect.centerx + (self.pointer_radius * math.cos(self.firing_angle*(math.pi/180)))
		p_ypos = self.magic_orb.rect.centery + (self.pointer_radius * math.sin(self.firing_angle*(math.pi/180)))
